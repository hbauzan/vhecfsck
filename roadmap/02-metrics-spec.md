...
5. Compute a 95% interval by **bootstrap resampling over queries** (default 1,000
   ...
   ([ADR-0003](adr/0003-empirical-metrics-only.md)).
...
- Default thresholds are calibrated at `|S| = 20_000, k_hub = 10` ([P8](phases/phase-8-calibration-and-hardening.md)).
...
### 4.3 Companion sub-check: entry-point health (post-MVP, [P7](phases/phase-7-qdrant-and-pgvector-adapters.md))
...
| :--- | :--- | :--- |
| HNSW in-degree distribution / unreachable-node fraction | Directly quantifies path blocking rather than inferring it from tombstone ratios | Graph-level adapter introspection ([P7](phases/phase-7-qdrant-and-pgvector-adapters.md)) |
...
| Temporal drift / trend | Turns a point-in-time check into an early-warning signal | Baseline persistence ([P8](phases/phase-8-calibration-and-hardening.md)) |
